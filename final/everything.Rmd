...
### Fitting Model with all predictors (Logit Model)

```{r}
m2<-  glm(credit_risk ~ ., data = dat, family = "binomial")
summary(m2)
```
### Feature selection
```{r include=FALSE}
step(m2, test = "Chisq", direction = "backward")
```
Comment: 
Feature selected:  -------- Pragith/Ethan
Null Deviance:	-------- Pragith/Ethan    
Residual Deviance -------- Pragith/Ethan

### Fitting the new reduced model (Based on last step)

```{r}
m22<-  glm(formula = credit_risk ~ status + duration + credit_history + 
    purpose + amount + savings + employment_duration + installment_rate + 
    personal_status_sex + other_debtors + present_residence + 
    age + other_installment_plans + housing + foreign_worker, 
    family = "binomial", data = dat)
summary(m22)
```


# Want to find a way to reduce our model so it only contains significant variables. So lets make changes to our df1 dataframe and work with that
...
```
### Fitting model now (same as model m2 from b4)
...
#Fitting new model based on steps fcn

```{r}
new3<-  glm(formula = credit_risk ~ status_less_than_0_DM + status_less_than_200_DM + 
    status_greater_than_200_DM + duration + credit_history_nocredits + 
    credit_history_existingcredits_paidback + credit_history_all_paidback + 
    ...
    otherdebtors_guarantor + presres_less_4 + age + instplan_none + 
    housing_rent + foreign_worker, family = "binomial", data = df1)
summary(new3)
```


### Lets add interaction terms

```{r}
new5<-  glm(formula = credit_risk ~ status_less_than_0_DM + status_less_than_200_DM + 
    status_greater_than_200_DM + duration + credit_history_nocredits + 
...
#### Std Pearson resid (outlier if abs value is greater than 3)

```{r}
std_pearson_resid <- rstandard(new5,type = "pearson")
which(abs(std_pearson_resid) > 3)
```
17 observations are outliers: lets remove them
...
#### Making adjustments to our new reduced dataset
...
# Fitting model after removing residuals (Huge IMPROVEMENT!). 

```{r}
new6<-  glm(formula = credit_risk ~ status_less_than_0_DM + status_less_than_200_DM + 
    status_greater_than_200_DM + duration + credit_history_nocredits + 
    credit_history_existingcredits_paidback + 
    purpose_newcar + purpose_usedcar + purpose_furniture + purpose_vacation + 
    purpose_retrain + purpose_bus + amount + savings_lessthan_1000DM + 
...
comparing m22 to this model:

* Residual Deviance went from 902.13 to 787

* AIC went from 990 to 845



# Lets check if the Reduced Logit model was a good fit based on dif tests:

```{r}
hoslem.test(new6$y, fitted(new6),g = 30)
```
Fail to reject null meaning Data fits the data well!
...
Sensitivity = 538/(162+538)
Sensitivity

Specificity = 63 / (63 + 220)
Specificity

Concordance_rate = (538 + 220) / (538 + 220 + 63 + 162)
Concordance_rate
```


Pretty High value for Sensitivity and Concordance rate which is great news!
...
```{r}
plot(m22$residuals ~ m22$fitted.values, data = df1)
abline(h = 0)
```
...
### Testing different models with data partition on the original dataset without feature selection. 
